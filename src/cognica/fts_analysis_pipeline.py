...
from cognica.protobuf import (
    fts_analysis_pipeline_pb2, fts_analysis_pipeline_pb2_grpc
)
...
class FTSAnalysisPipeline:
    ...
    def execute(self, collection_name: str, index_name: str, query: str,
                field_names: t.Optional[t.List[str]] = None) -> str:
        req = PipelineExecutionRequest(
            collection_name=collection_name, index_name=index_name,
            field_names=field_names, query=query)
        resp: PipelineExecutionResponse = self._invoke(
            self._stub.execute, req, wait_for_ready=True)

        return resp.result

    def execute_adhoc(self, pipeline_def: str, query: str,
                      field_names: t.Optional[t.List[str]] = None) -> str:
